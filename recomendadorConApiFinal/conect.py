...
class JuezDB:
    ...
    #Me devuelve la posición donde debería insertarse de manera ordenada un nuevo valor
    #El algoritmo NO está nada optimizado.
    def _obtenerPosToInsert(self, id):
        pos = -1
        lista = self._users
    ...
    def actualizarEntregas(self,nuevasEntregas, ultimaEntrega):
        ...
        for elem in nuevasEntregas:
            ...
            if self.obtenerPosUser(userId) == -1: #TODO: testear nuevamente esto. Quizás esta insertando de más
                ...
                posicionAinsertar = self._obtenerPosToInsert(userId)
                compUser = False
                # añadir usuario nuevo al final de parseador de usuarios
                self._users = np.insert(self._users, posicionAinsertar,userId)
                ...
                self.matrizDatos = np.insert(self.matrizDatos, posicionAinsertar, problemas, 0)
