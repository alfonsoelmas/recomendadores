...
while externo < maxSize:
        interno = externo + 1
        while interno < maxSize:
                recomendador = RecomendadorBasico(listaTodosUsuarios[externo])
                correl = recomendador.correlacion(listaTodosUsuarios[interno])
                interno = interno + 1
                print(correl)
                if correl!=0:
                        g.add_edge(interno, externo, weight=correl*5)
        externo = externo + 1
G=g       
#G = nx.MultiGraph()
#i=0
...
#        listadoPesos.append(d['weight'])
#        i = 1 +1
#nx.draw_networkx(G, pos, node_width=10,node_color='b', edgelist=edges, width=1.0)
nx.draw_circular(G, node_weight=100, node_color='#A0CBE2', width=weights)
#nx.draw(g)
#plt.show()

plt.savefig("nodos20UsersTamRelCircular.png")
print("llegue")
