                ...
                if resultados["error_get"] == 0:
                    for i in resultados["submission"]:
                        if i["num"] == ultimoSubmitRecomender:
                            ...
                            dictData = {}
                            dictData["userID"]      = int(userId)
                            dictData["problemPos"]  = int(problemId)-100 #TODO, creo que corresponde a la posicion del problema ya parseado, pero restandole 100 o algo así (El external ID)
